...
export interface MeshData {
  ...
  /** UV coordinate pool — its own vertex space (length = uvVertexCount * 2). */
  uvs?: Float32Array;
  /** Triangle list into `uvs`, parallel to `indices`. When omitted, UV corners mirror `indices`. */
  uvIndices?: Uint32Array;
  indices: Uint32Array;    // triangle list, length = triCount * 3
  /** Logical face groups — each lists triangle indices (into `faces` after conversion). */
  faceGroups?: number[][];
}
